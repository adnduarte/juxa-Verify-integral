...
export const DashboardLayout: React.FC<DashboardLayoutProps> = ({
  ...
  const isGeminiConfigured = (() => {
    ...
    if (override && override.length > 10) return true;
    ...
    <div className="flex h-screen bg-slate-50 text-slate-900 font-sans overflow-hidden w-full">
      ...
      <aside className={`hidden md:flex flex-col bg-white border-r border-slate-200 h-full flex-shrink-0 no-print transition-all duration-300 ${isCollapsed ? 'w-20' : 'w-64'}`}>
        ...
        <div className="p-4 border-t border-slate-200">
          ...
          {!isCollapsed && (
            <div className={`mb-4 p-2 rounded-lg text-[10px] font-bold flex flex-col gap-2 border ${
              isGeminiConfigured ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-red-50 text-red-700 border-red-100'
            }`}>
              <div className="flex items-center gap-2">
                <div className={`w-2 h-2 rounded-full ${isGeminiConfigured ? 'bg-emerald-500 animate-pulse' : 'bg-red-500'}`} />
                {isGeminiConfigured ? 'GEMINI IA: ACTIVA' : 'GEMINI IA: ERROR CONFIG'}
              </div>
              {!isGeminiConfigured && (
                <button 
                  onClick={() => {
                    const key = prompt('Introduce tu GEMINI_API_KEY manualmente:');
                    if (key && key.length > 10) {
                      localStorage.setItem('GEMINI_API_KEY_OVERRIDE', key);
                      window.location.reload();
                    }
                  }}
                  className="w-full py-1 bg-red-600 text-white rounded text-[9px] hover:bg-red-700 transition-colors"
                >
                  FORZAR CLAVE (FIX)
                </button>
              )}
